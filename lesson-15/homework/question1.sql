...
select @columns=string_agg(quotename(column_name), ',') --get all columns
from (
	select column_name
	from information_schema.columns
	where column_name not in ('identifier_name') ---except from identifier_name
	and table_name='contacts'
	) as cols;
set @sql='select
		  identifier_name,
		  (select'+@columns+'for json path, without_array_wrapper) as properties 
		  from contacts	 for json auto';
			---for json auto is used for get json format inside of value
